...
2. テキストの前処理 (Text Pre-processing)
 - 抽出されたテキストは、**processTextWithSettings**という中心的な関数に渡されます。
 - この関数内で、以下の処理が順番に適用されます。
 - 話者分離: 設定されたトリガー文字列（例: 「」や:）に基づいて、テキストが複数の話者ごとのチャンクに分割されます。
 - テキスト分割と文末削除: 分割された各テキストチャンクに対し、**！、？、。」**といった記号を区切りとして、さらに細かい文の単位に分割されます。この処理と同時に、文末の不要な文字を削除する正規表現処理が適用されます。

3. 空テキストの処理 (Handling Empty Text)
 - 文末削除の処理後、テキストチャンクが空になった場合（例: 「。」というテキストが削除された後）、ハイライトとのずれをなくすために、そのテキストを半角スペースに変換します。
 - この処理によって、音声合成ソフトへの入力が途切れることなく、Webページ上の対応するテキスト要素との同期を維持したまま、スムーズに次の読み上げへと移行することができます。

4. 読み上げキューの作成 (Creating the Speech Queue)
...
1. 読み上げキューへの追加 (Adding to the Speech Queue)
...
2. 非同期リクエストの開始 (Initiating Asynchronous Request)
